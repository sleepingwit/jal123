...
_disttree = {'': OptFile('%(readme)s', 'README.txt'),
             ...
             'bin': (File('%(blddir)s/svn/svn.exe'),
                     ...
                     OptFile('%(blddir)s/../contrib/client-side/svn-push/svn-push.exe'),
                     OptFile('%(blddir)s/../contrib/client-side/svn-push/svn-push.pdb'),
                     ...
                     OptFile('%(blddir)s/mod_dav_svn/mod_dav_svn.so'),
                     OptFile('%(blddir)s/mod_dav_svn/mod_dav_svn.pdb'),
                     OptFile('%(blddir)s/mod_authz_svn/mod_authz_svn.so'),
                     OptFile('%(blddir)s/mod_authz_svn/mod_authz_svn.pdb'),
                     FileGlob('%(blddir)s/libsvn_*/libsvn_*.dll'),
                     FileGlob('%(blddir)s/libsvn_*/libsvn_*.pdb'),
                     OptFile('%(@apr)s/%(aprrel)s/libapr-1.dll'),
                     OptFile('%(@apr)s/%(aprrel)s/libapr-1.pdb'),
                     OptFile('%(@apr-iconv)s/%(aprrel)s/libapriconv-1.dll'),
                     OptFile('%(@apr-iconv)s/%(aprrel)s/libapriconv-1.pdb'),
                     OptFile('%(@apr-util)s/%(aprrel)s/libaprutil-1.dll'),
                     OptFile('%(@apr-util)s/%(aprrel)s/libaprutil-1.pdb'),
                     OptFile('%(@berkeley-db)s/bin/libdb%(bdbver)s.dll'),
                     OptFile('%(@sasl)s/lib/libsasl.dll'),
                     OptFile('%(@sasl)s/lib/libsasl.pdb'),
                     OptFile('%(@sasl)s/utils/pluginviewer.exe'),
                     OptFile('%(@sasl)s/utils/pluginviewer.pdb'),
                     OptFile('%(@sasl)s/utils/sasldblistusers2.exe'),
                     OptFile('%(@sasl)s/utils/sasldblistusers2.pdb'),
                     OptFile('%(@sasl)s/utils/saslpasswd2.exe'),
                     OptFile('%(@sasl)s/utils/saslpasswd2.pdb'),
             ...
             'lib/apr': OptFile('%(@apr)s/%(aprrel)s/libapr-1.lib'),
             'lib/apr-iconv': OptFile('%(@apr-iconv)s/%(aprrel)s/libapriconv-1.lib'),
             'lib/apr-util': (OptFile('%(@apr-util)s/%(aprrel)s/libaprutil-1.lib'),
                              OptFile('%(@apr-util)s/%(aprxml)s/xml.lib'),
                              OptFile('%(@apr-util)s/%(aprxml)s/xml.pdb'),
                              ),

             'lib/serf': (OptFile('%(@serf)s/Release/serf.lib'),
                          ),

             'lib/sasl': (OptFile('%(@sasl)s/lib/libsasl.lib'),
                          OptFile('%(@sasl)s/lib/libsasl.pdb'),
                          ),

             'licenses': None,
             'licenses/bdb': OptFile('%(@berkeley-db)s/LICENSE'),
             'licenses/serf': OptFile('%(@serf)s/LICENSE'),
             'licenses/zlib': OptFile('%(@zlib)s/README'),
             'licenses/apr-util': (OptFile('%(@apr-util)s/LICENSE'),
                                   OptFile('%(@apr-util)s/NOTICE'),
                                   ),
             'licenses/apr-iconv': (OptFile('%(@apr-iconv)s/LICENSE'),
                                    OptFile('%(@apr-iconv)s/NOTICE'),
                                    ),
             'licenses/apr': (OptFile('%(@apr)s/LICENSE'),
                              OptFile('%(@apr)s/NOTICE'),
                              ),
             'licenses/openssl': OptFile('%(@openssl)s/LICENSE'),
             'licenses/svn' : OptFile('%(srcdir)s/COPYING'),
             'licenses/cyrus-sasl' : OptFile('%(@sasl)s/COPYING'),
...
for module in ('Client', 'Core', 'Delta', 'Fs', 'Ra', 'Repos', 'Wc'):
  _disttree['perl/site/lib/auto/SVN/_' + module] = (
    OptFile('%(binddir)s/swig/perl/native/_' + module + '.dll'),
    OptFile('%(binddir)s/swig/perl/native/_' + module + '.pdb'))
